...
def get_data_bas_location(location, df):
        ...
        #result = ["", location, ""] + list(df[df['Country/Region'].str.contains(location)].groupby("Country/Region"))
        #return result
        #return ["",location,""]+ list(df[df['Country/Region'].str.contains(location)].values()#.groupby("Country/Region"))#.sum().values[0])
        #return df[df['Country/Region'].str.contains(location)].values
    try:
